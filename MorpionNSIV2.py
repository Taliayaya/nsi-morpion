    ...
    def dessiner(self, value): #-1 Pour rien, 0 pour Croix, 1 pour Rond
        ...
        if not(value):
            ...
            pg.draw.circle(surf,COULEUR_DESSIN,(self.width/2 + self.topleft[0] + 1,\
                           ...
                           (min(self.height,self.widt)/2-6), 5)
        ...
        for event in pg.event.get():
            if event.type == pg.KEYDOWN:
                if event.unicode.isnumeric() :
                    ...
                    display = smallfont.render(nombre_actuel,False,(0,255,0))
...
def init():
    ...
    def Dessiner_options():
        fenetrerejouer = True
        surf.fill(COULEUR_FOND)
        text = smallfont.render("Pour jouer, entrez <espace>. Pour quitter, entrez <entree>.",False,(0,255,0))
            ...
            for event in pg.event.get():
                if event.type == pg.KEYDOWN:
                     ...
                     if event.key == pg.K_SPACE:
                        surf.fill(COULEUR_FOND)
                        text = smallfont.render("Veuillez entrer le nombre de lignes",False,(0,255,0))
                        surf.blit(text, (180,200))
                        hauteur = wait_input()
                        # Récupération du nombre de lignes
                        surf.fill(COULEUR_FOND)
                        text = smallfont.render("Veuillez entrer le nombre de colonnes",False,(0,255,0))
                        surf.blit(text, (180,200))
                        largeur = wait_input()
                        # Recupération du nombre de colonnes
                        surf.fill(COULEUR_FOND)
                        text = smallfont.render("Veuillez entrer le nombre de symboles à aligner pour gagner",False,(0,255,0))
                        surf.blit(text, (50,200))
                        wincond = wait_input()
                        # Récupération de la condition de victoire
                        surf.fill(COULEUR_FOND)
                        run(hauteur,largeur,wincond)
                        text = smallfont.render("Pour rejouer, entrez <espace>. Pour quitter, entrez <entree>.",False,(0,255,0))
                        surf.blit(text, (50,200))
